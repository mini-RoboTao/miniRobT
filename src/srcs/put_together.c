...
/*   Updated: 2023/08/09 17:21:05 by dapaulin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_shape	left_cylinder(void *v)
{
	...
	s3 = new_cylinder();
	s3.cylinder->transform = multiply_matrix(\
									translation(-1.5, 0.33, -0.75), \
									scaling(0.33, 0.33, 0.33), 4, 4);
	s3.cylinder->material = new_material();
	s3.cylinder->material.color = fill_color(1, 0.8, 0.1);
	s3.cylinder->material.diffuse = 0.7;
	s3.cylinder->material.specular = 0.3;
	pattern = stripe_pattern(fill_color(1, 1, 0), fill_color(1, 0.4, 0));
	set_pattern_transform(&pattern, scaling(0.5, 0.5, 0.5));
	s3.cylinder->material.pattern = pattern;
	return (s3);
	(void)v;
}
