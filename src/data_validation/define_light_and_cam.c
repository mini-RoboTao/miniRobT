...
/*   Updated: 2023/08/17 23:27:46 by dapaulin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_define_a_light	define_ambient_light(char **params, t_world *world)
{
	...
	if (ft_arraylen(params) != 3)
		clean_parser_error(*world, params, "error code: 57 - \
	...
	if (a_light.intensity < 0 || a_light.intensity > 1)
		clean_parser_error(*world, params, "error code: 97 - \
		Number out of range");
...
t_define_light	define_light(char **params, t_world *world)
{
	...
	if (ft_arraylen(params) != 4)
		clean_parser_error(*world, params, "error code: 60 - \
		Invalid quantity arguments");
	...
	if (light.intensity < 0 || light.intensity > 1)
		clean_parser_error(*world, params, "error code: 98 - \
		Number out of range");
...
t_define_cam	define_camera(char **params, t_world *world)
{
	...
	cam.hsize = HEIGHT;
	if (ft_arraylen(params) != 4)
		clean_parser_error(*world, params, "erro code: 11 - \
		Invalid quantity arguments");
